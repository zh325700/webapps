...
if (isset($_POST['username'], $_POST['email'], $_POST['p'], $_POST['admin'])) {
    ...
    $prep_stmt = "SELECT ID_Caregiver FROM Caregiver WHERE email = ? LIMIT 1";
    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO Caregiver (username, email, password, salt, permission) VALUES (?, ?, ?, ?, ?)")) {
